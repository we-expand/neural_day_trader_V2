...
describe('MetatraderAccountApi', () => {
   ...
   * @test {MetatraderAccount#enableMetaStatsApi}
   */
  it('should enable account copy factory API', async () => {
    getAccountStub
      .onFirstCall().resolves({
        _id: 'id',
        login: '50194988',
    ...
    const accountId = 'id';
    const copyFactoryRoles = ['PROVIDER'];
    const copyFactoryResourceSlots = 1;
    sandbox.stub(client, 'enableCopyFactoryApi').resolves();
    let account = await api.getAccount(accountId);
    await account.enableCopyFactoryApi(copyFactoryRoles, copyFactoryResourceSlots);
    account.copyFactoryRoles.should.be.deepEqual(['PROVIDER']);
    account.copyFactoryResourceSlots.should.be.eql(1);
    sinon.assert.calledWith(client.enableCopyFactoryApi, accountId, copyFactoryRoles, copyFactoryResourceSlots);
    sinon.assert.calledWith(client.getAccount, accountId);
    sinon.assert.calledTwice(client.getAccount);
  });
