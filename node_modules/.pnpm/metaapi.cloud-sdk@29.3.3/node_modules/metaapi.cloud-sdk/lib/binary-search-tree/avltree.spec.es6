...
describe('AVL tree', function () {
  ...
  describe('Insertion', function () {
    ...
    it('Insert at the root if its the first insertion', function () {
      ...
      should(avlt.tree.left).be.null();
      should(avlt.tree.right).be.null();
    });
  ...
  describe('Search', function () {
    ...
    it('Can search for data between two bounds', function () {
      ...
      should.deepEqual(avlt.betweenBounds({ $gte: 8, $lte: 15 }), ['data 8', 'data 10', 'data 13', 'data 15']);
      should.deepEqual(avlt.betweenBounds({ $gt: 8, $lt: 15 }), ['data 10', 'data 13']);
    });
    ...
    it('Bounded search can handle cases where query contains both $lt and $lte, or both $gt and $gte', function () {
      ...
      should.deepEqual(avlt.betweenBounds({ $gt:8, $gte: 8, $lte: 15 }), ['data 10', 'data 13', 'data 15']);
      should.deepEqual(avlt.betweenBounds({ $gt:5, $gte: 8, $lte: 15 }), ['data 8', 'data 10', 'data 13', 'data 15']);
      should.deepEqual(avlt.betweenBounds({ $gt:8, $gte: 5, $lte: 15 }), ['data 10', 'data 13', 'data 15']);

      should.deepEqual(avlt.betweenBounds({ $gte: 8, $lte: 15, $lt: 15 }), ['data 8', 'data 10', 'data 13']);
      should.deepEqual(avlt.betweenBounds({ $gte: 8, $lte: 18, $lt: 15 }), ['data 8', 'data 10', 'data 13']);
      should.deepEqual(avlt.betweenBounds({ $gte: 8, $lte: 15, $lt: 18 }), ['data 8', 'data 10', 'data 13', 'data 15']);
    });
    ...
    it('Bounded search can work when one or both boundaries are missing', function () {
      ...
      should.deepEqual(avlt.betweenBounds({ $gte: 11 }), ['data 13', 'data 15', 'data 18']);
      should.deepEqual(avlt.betweenBounds({ $lte: 9 }), ['data 3', 'data 5', 'data 8']);
    });
  ...
  describe('Deletion', function () {
    ...
    it('Deletion does nothing on an empty tree', function () {
      ...
      Object.prototype.hasOwnProperty.call(avlt, 'key').should.equal(false)
      Object.prototype.hasOwnProperty.call(avltu, 'key').should.equal(false)
    ...
    it('If no value is provided, it will delete the entire node even if there ' + 
       'are multiple pieces of data', function () {
      let avlt = new AVLTree();
      ...
      should.deepEqual(avlt.search(5), ['hello', 'world']);
    ...
    it('Can remove only one value from an array', function () {
      ...
      avlt.delete(5, 'hello');
      avlt.checkIsAVLT();
      should.deepEqual(avlt.search(5), ['world']);
      avlt.getNumberOfKeys().should.equal(4);
    });
    ...
    it('If value provided but node contains only one value, remove entire node', function () {
      ...
      should.deepEqual(avlt.search(3), ['yes2']);
      avlt.getNumberOfKeys().should.equal(4);
    ...
    it('Can remove the root from a tree with height 2 when the root has two children (special case)', function () {
      ...
      avlt.checkIsAVLT();
      avlt.getNumberOfKeys().should.equal(2);
      should.deepEqual(avlt.search(5), ['no']);
      should.deepEqual(avlt.search(15), ['yes']);
    });

    it('Can remove the root from a tree with height 3 when the root has ' + 
       'two children (special case where the two children themselves have children)', function () {
      let avlt = new AVLTree();
    ...
    it('Removing falsy values does not delete the entire key', function () {
      ...
      avlt.insert(10, 2);
      avlt.insert(10, 1);
      should.deepEqual(avlt.search(10), [2, 1]);

      avlt.delete(10, 2);
      should.deepEqual(avlt.search(10), [1]);

      avlt.insert(10, 0);
      should.deepEqual(avlt.search(10), [1, 0]);
  ...
  it('Can use undefined as key and value', function () {
    ...
    avlt.checkIsAVLT();
    avlt.getNumberOfKeys().should.equal(1);
    should.deepEqual(avlt.search(2), [undefined]);
    should.deepEqual(avlt.search(undefined), []);

    avlt.insert(undefined, 'hello');
    avlt.checkIsAVLT();
    avlt.getNumberOfKeys().should.equal(2);
    should.deepEqual(avlt.search(2), [undefined]);
    should.deepEqual(avlt.search(undefined), ['hello']);

    avlt.insert(undefined, 'world');
    avlt.checkIsAVLT();
    avlt.getNumberOfKeys().should.equal(2);
    should.deepEqual(avlt.search(2), [undefined]);
    should.deepEqual(avlt.search(undefined), ['hello', 'world']);

    avlt.insert(4, undefined);
    avlt.checkIsAVLT();
    avlt.getNumberOfKeys().should.equal(3);
    should.deepEqual(avlt.search(2), [undefined]);
    should.deepEqual(avlt.search(4), [undefined]);
    should.deepEqual(avlt.search(undefined), ['hello', 'world']);
    ...
    should.deepEqual(avlt.search(2), [undefined]);
    should.deepEqual(avlt.search(4), [undefined]);
    should.deepEqual(avlt.search(undefined), ['world']);
    ...
    avlt.checkIsAVLT();
    avlt.getNumberOfKeys().should.equal(1);
    should.deepEqual(avlt.search(2), []);
    should.deepEqual(avlt.search(4), [undefined]);
    should.deepEqual(avlt.search(undefined), []);
    ...
    avlt.getNumberOfKeys().should.equal(0);
    should.deepEqual(avlt.search(2), []);
    should.deepEqual(avlt.search(4), []);
    should.deepEqual(avlt.search(undefined), []);
  });
  ...
  it('Can use null as key and value', function () {
    ...
    avlt.checkIsAVLT();
    avlt.getNumberOfKeys().should.equal(1);
    should.deepEqual(avlt.search(2), [null]);
    should.deepEqual(avlt.search(null), []);

    avlt.insert(null, 'hello');
    avlt.checkIsAVLT();
    avlt.getNumberOfKeys().should.equal(2);
    should.deepEqual(avlt.search(2), [null]);
    should.deepEqual(avlt.search(null), ['hello']);

    avlt.insert(null, 'world');
    avlt.checkIsAVLT();
    avlt.getNumberOfKeys().should.equal(2);
    should.deepEqual(avlt.search(2), [null]);
    should.deepEqual(avlt.search(null), ['hello', 'world']);

    avlt.insert(4, null);
    avlt.checkIsAVLT();
    avlt.getNumberOfKeys().should.equal(3);
    should.deepEqual(avlt.search(2), [null]);
    should.deepEqual(avlt.search(4), [null]);
    should.deepEqual(avlt.search(null), ['hello', 'world']);
    ...
    should.deepEqual(avlt.search(2), [null]);
    should.deepEqual(avlt.search(4), [null]);
    should.deepEqual(avlt.search(null), ['world']);
  ...
  describe('Execute on every node (=tree traversal)', function () {
    ...
    it('Can execute a function on every node', function () {
      ...
      should.deepEqual(keys, [3, 5, 8, 10, 11, 15, 159]);
  ...
  describe('Randomized test (takes much longer than the rest of the test suite)', function () {
    ...
    function checkDataIsTheSame (avlt, data) {
      ...
      _.reduce(_.map(data, function (d) { return d.length; }), 
        function (memo, n) { return memo + n; }, 0).should.equal(avltDataElems.length);
    ...
    function checkDataEquality (fromavlt, fromData) {
      ...
      should.deepEqual(fromavlt, fromData);
    }
