...
describe('Binary search tree', function () {
  ...
  it('Upon creation, left, right are null, key and data can be set', function () {
    ...
    should(bst.left).be.null()
    should(bst.right).be.null()
  ...
  describe('Insertion', function () {
    ...
    it('Insert on the left if key is less than the root\'s', function () {
      ...
      should(bst.left.left).be.null()
      should(bst.left.right).be.null()
    });
    ...
    it('Insert on the right if key is greater than the root\'s', function () {
      ...
      should(bst.right.left).be.null()
      should(bst.right.right).be.null()
    });
  ...
  describe('Search', function () {
    ...
    it('Can search for data between two bounds', function () {
      ...
      should.deepEqual(bst.betweenBounds({ $gte: 8, $lte: 15 }), ['data 8', 'data 10', 'data 13', 'data 15']);
      should.deepEqual(bst.betweenBounds({ $gt: 8, $lt: 15 }), ['data 10', 'data 13']);
    });
    ...
    it('Bounded search can handle cases where query contains both $lt and $lte, or both $gt and $gte', function () {
      ...
      should.deepEqual(bst.betweenBounds({ $gt:8, $gte: 8, $lte: 15 }), ['data 10', 'data 13', 'data 15']);
      should.deepEqual(bst.betweenBounds({ $gt:5, $gte: 8, $lte: 15 }), ['data 8', 'data 10', 'data 13', 'data 15']);
      should.deepEqual(bst.betweenBounds({ $gt:8, $gte: 5, $lte: 15 }), ['data 10', 'data 13', 'data 15']);

      should.deepEqual(bst.betweenBounds({ $gte: 8, $lte: 15, $lt: 15 }), ['data 8', 'data 10', 'data 13']);
      should.deepEqual(bst.betweenBounds({ $gte: 8, $lte: 18, $lt: 15 }), ['data 8', 'data 10', 'data 13']);
      should.deepEqual(bst.betweenBounds({ $gte: 8, $lte: 15, $lt: 18 }), ['data 8', 'data 10', 'data 13', 'data 15']);
    });
    ...
    it('Bounded search can work when one or both boundaries are missing', function () {
      ...
      should.deepEqual(bst.betweenBounds({ $gte: 11 }), ['data 13', 'data 15', 'data 18']);
      should.deepEqual(bst.betweenBounds({ $lte: 9 }), ['data 3', 'data 5', 'data 8']);
    });
  ...
  describe('Deletion', function () {
    ...
    it('If no value is provided, it will delete the entire node even if there ' + 
      ...
      should.deepEqual(bst.search(5), ['hello', 'world']);
      bst.getNumberOfKeys().should.equal(4);
    ...
    it('Can remove only one value from an array', function () {
      ...
      bst.delete(5, 'hello');
      should.deepEqual(bst.search(5), ['world']);
      bst.getNumberOfKeys().should.equal(4);
    });
    ...
    it('If value provided but node contains only one value, remove entire node', function () {
      ...
      should.deepEqual(bst.search(3), ['yes2']);
      bst.getNumberOfKeys().should.equal(4);
    ...
    it('Can remove the root from a tree with height 2 when the root has two children (special case)', function () {
      ...
      bst.checkIsBST();
      bst.getNumberOfKeys().should.equal(2);
      should.deepEqual(bst.search(5), ['no']);
      should.deepEqual(bst.search(15), ['yes']);
    });

    it('Can remove the root from a tree with height 3 when the root has two children ' + 
       '(special case where the two children themselves have children)', function () {
      let bst = new BinarySearchTree();
  ...
  it('Can use undefined as key and value', function () {
    ...
    bst.checkIsBST();
    bst.getNumberOfKeys().should.equal(1);
    should.deepEqual(bst.search(2), [undefined]);
    should.deepEqual(bst.search(undefined), []);

    bst.insert(undefined, 'hello');
    bst.checkIsBST();
    bst.getNumberOfKeys().should.equal(2);
    should.deepEqual(bst.search(2), [undefined]);
    should.deepEqual(bst.search(undefined), ['hello']);

    bst.insert(undefined, 'world');
    bst.checkIsBST();
    bst.getNumberOfKeys().should.equal(2);
    should.deepEqual(bst.search(2), [undefined]);
    should.deepEqual(bst.search(undefined), ['hello', 'world']);

    bst.insert(4, undefined);
    bst.checkIsBST();
    bst.getNumberOfKeys().should.equal(3);
    should.deepEqual(bst.search(2), [undefined]);
    should.deepEqual(bst.search(4), [undefined]);
    should.deepEqual(bst.search(undefined), ['hello', 'world']);
    ...
    should.deepEqual(bst.search(2), [undefined]);
    should.deepEqual(bst.search(4), [undefined]);
    should.deepEqual(bst.search(undefined), ['world']);
  ...
  it('Can use null as key and value', function () {
    ...
    bst.checkIsBST();
    bst.getNumberOfKeys().should.equal(1);
    should.deepEqual(bst.search(2), [null]);
    should.deepEqual(bst.search(null), []);

    bst.insert(null, 'hello');
    bst.checkIsBST();
    bst.getNumberOfKeys().should.equal(2);
    should.deepEqual(bst.search(2), [null]);
    should.deepEqual(bst.search(null), ['hello']);

    bst.insert(null, 'world');
    bst.checkIsBST();
    bst.getNumberOfKeys().should.equal(2);
    should.deepEqual(bst.search(2), [null]);
    should.deepEqual(bst.search(null), ['hello', 'world']);

    bst.insert(4, null);
    bst.checkIsBST();
    bst.getNumberOfKeys().should.equal(3);
    should.deepEqual(bst.search(2), [null]);
    should.deepEqual(bst.search(4), [null]);
    should.deepEqual(bst.search(null), ['hello', 'world']);
    ...
    should.deepEqual(bst.search(2), [null]);
    should.deepEqual(bst.search(4), [null]);
    should.deepEqual(bst.search(null), ['world']);
  ...
  describe('Randomized test (takes much longer than the rest of the test suite)', function () {
    ...
    function checkDataIsTheSame (bst, data) {
      ...
      _.reduce(_.map(data, function (d) { return d.length; }), 
        function (memo, n) { return memo + n; }, 0).should.equal(bstDataElems.length);
    ...
    function checkDataEquality (fromBst, fromData) {
      ...
      should.deepEqual(fromBst, fromData);
    }
