...
describe('MetatraderAccountClient', () => {
   ...
   * @test {MetatraderAccountClient#getAccounts}
   */
  it('should retrieve MetaTrader accounts from API using exact api-version', async () => {
    let expected = [{
      _id: '1eda642a-a9a3-457c-99af-3bc5e8d5c4c9',
      login: '50194988',
      name: 'mt5a',
  ...
  /**
   * @test {MetatraderAccountClient#enableCopyFactoryApi}
   */
  it('should enable copy factory API', async () => {
    const accountId = 'id';
    const copyFactoryRoles = ['PROVIDER'];
    const copyFactoryResourceSlots = 1;
    await accountClient.enableCopyFactoryApi(accountId, copyFactoryRoles, copyFactoryResourceSlots);
    sinon.assert.calledOnceWithExactly(httpClient.request, {
      url: `${provisioningApiUrl}/users/current/accounts/id/enable-copy-factory-api`,
      method: 'POST',
      headers: {
        'auth-token': token
      },
    ...
    try {
      ...
      error.message.should.equal(
        'You can not invoke enableMetaStatsApi method, because you have connected with account ' +
          'access token. Please use API access token from https://app.metaapi.cloud/api-access/generate-token page ' + 
          'to invoke this method.'
      );
    }
  });
