> **Arquivo de arquivo (não carregado automaticamente).** Histórico completo de sessões até 2026-07-24, preservado sem edição, movido pra fora do CLAUDE.md pra reduzir o custo de contexto de cada sessão nova (o CLAUDE.md tinha crescido pra ~493KB / ~123 mil tokens estimados, lido do zero em toda janela nova). Estado atual e decisões vivas ficam em CLAUDE.md (raiz do projeto) e research/AI_BRAIN_SPEC.md (motor de decisão da IA). Consulte este arquivo só quando precisar do detalhe de uma investigação/bug antigo específico.
